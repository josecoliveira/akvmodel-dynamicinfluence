...
class AKV:
    ...
    Args:
        ...
        update_function(Callable[[List[List[float]], List[List[float]]], List[List[float]]):
            A function the gets a belief_state and an influence_graph and returns a new belief_state.
    ...
    Attributes:
        ...
        states (List[List[List[float]]]): List of all belief states computed so far using the update function.
    """
    ...
    def _check_sizes(self):
        ...
        Raises:
            Exception: If the number of beliefs and the number of nodes in the influence graph are different.
    ...
    def __init__(
        ...
        self.belief_state_history = np.array([belief_state])
        self.influence_graph_history = np.array([influence_graph])
    ...
    def update(self) -> List[List[float]]:
        ...
        """
        new_belief_state, new_influence_graph = self.update_function(
            self.belief_state, self.influence_graph
        )
        self.belief_state = new_belief_state
        self.influence_graph = new_influence_graph
        self.belief_state_history = np.vstack(
            (self.belief_state_history, [self.belief_state])
        )
        self.influence_graph_history = np.vstack(
            (self.influence_graph_history, [self.influence_graph])
        )
    ...
    def get_polarization(
        ...
        Args:
            ...
            K (float, optional): Hyperparameter K of the Esteban-Ray measure. Defaults to 1000.
            alpha (float, optional): Hyperparameter $\alpha$ of the Esteban-Ray measure. Defaults to 1.6.
        ...
        return [
            [
                polarization(belief_state[i])
                for belief_state in self.belief_state_history
            ]
...
class InfluenceGraphs:
    ...
    def clique(num_agents: int, influence: float = 1) -> List[List[float]]:
        r"""Creates a clique influence graph.
        ...
        Args:
            ...
            influence (float, optional): Influence that every agent has on each other. Defaults to 1.
        ...
        def I(i: int, j: int) -> float:
            return 1 if i == j else influence
    ...
    def circular(num_agents: int, influence: float = 0.5) -> List[List[float]]:
        r"""Creates a circular influence graph.
        ...
        Args:
            num_agents (int): Number of agents.
            influence (float, optional): Influence that an agent has on the next one. Defaults to 0.5.
    ...
    def faintly(
        ...
        r"""Creates a faintly communicating influence graph.
        ...
        Args:
            num_agents (int): Number of agents.
            strong_influence (float, optional): Influence between agents of the same group. Defaults to 0.5.
            weak_influence (float, optional): Influence between agents from different groups. Defaults to 0.1.
    ...
    def disconnected(num_agents: int, influence: float = 0.5) -> List[List[float]]:
        r"""Creates a disconnected influence graph.
        ...
        Args:
            num_agents (int): Number of agents.
            influence (float, optional): Influence between agents of the same  group. Defaults to 0.5.
    ...
    def malleable_influencers(
        ...
        r"""Creates a malleable influencers influence graph.
        ...
        Args:
            num_agents (int): Number of agents.
            influencer_1_influence (float, optional): Influence that agent 0 has on  other agents. Defaults to 0.8.
            influencer_2_influence (float, optional): Influence that agents n - 1 has  on other agents. Defaults to 0.4.
            influence_on_influencer_1 (float, optional): Influence that other agents  have on agent 0. Defaults to 0.1.
            influence_on_influencer_2 (float, optional): Influence that other agents  have on agent n - 1. Defaults to 0.1.
            other_agents_influence (float, optional): Influence that the other agents  have on each other. Defaults to 0.1.
    ...
    def unrelenting_influencers(
        ...
        r"""Creates an unrelenting influencers influence graph.
        ...
        Args:
            num_agents (int): Number of agents.
            influencer_1_influence (float, optional): Influence that agent 0 has on  other agents. Defaults to 0.6.
            influencer_2_influence (float, optional): Influence that agents n - 1 has  on other agents. Defaults to 0.6.
            other_agents_influence (float, optional): Influence that the other agents have on each other. Defaults to 0.1.
...
class UpdateFunctions:
    ...
    def single_influencer(
        ...
        # Influence array of agent 0 is the the belief state
        belief_state_0 = [
            np.copy(influence_graph[0]),
            [1 - influence_value for influence_value in influence_graph[0]],
        ]

        # Update the influence array using classic update function
        new_belief_array_0, _ = UpdateFunctions.classic(belief_state_0, influence_graph)

        # Influence of agent 0 is updated using the updated belief array
        influence_graph[0] = new_belief_array_0[0]

        # Influence of agent 0 on themselves does not change.
        influence_graph[0][0] = belief_state_0[0][0]

        return belief_state, influence_graph

    @staticmethod
    def all_dynamic_influence(
        belief_state: list[list[float]], influence_graph: list[list[float]]
    ) -> tuple[list[list[float]], list[list[float]]]:
        new_influence_graph: list[list[float]] = np.copy(influence_graph)

        for i in range(len(influence_graph)):
            belief_state_ = [
                np.copy(influence_graph[i]),
                [1 - influence_value for influence_value in influence_graph[i]],
            ]
            new_influence_array = UpdateFunctions.classic(
    ...
    @staticmethod
    def dynamic_influence_with_belief_update(
        belief_state: list[list[float]], influence_graph: list[list[float]]
    ) -> tuple[list[list[float]], list[list[float]]]:
        new_belief_state, _ = UpdateFunctions.classic(belief_state, influence_graph)

        return (
            UpdateFunctions.classic(belief_state, influence_graph)[0],
            UpdateFunctions.all_dynamic_influence(belief_state, influence_graph)[1],
        )
